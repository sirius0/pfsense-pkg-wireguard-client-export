...
- **Current preview:** pfSense CE 2.9.0 on FreeBSD 16 `amd64`, with the
  official WireGuard 0.2.13_4 package.
- **Legacy preview:** pfSense CE 2.7.2 on FreeBSD 14 `amd64`, with the
  official WireGuard 0.2.1 package.
- **Not validated:** pfSense CE 2.8.1.

Both published packages are experimental previews scoped to their exact tested
environment. The adapter fails closed when the installed WireGuard package
does not expose a tested contract. See
[`docs/COMPATIBILITY.md`](docs/COMPATIBILITY.md).

### Preview releases

| Release | Package | Validated environment | Status |
|---|---|---|---|
| `v0.1.0-beta.2` | `0.1.0.b2` | pfSense CE 2.9.0, FreeBSD 16 `amd64`, WireGuard 0.2.13_4 | Current experimental preview |
| `v0.1.0-beta.1` | `0.1.0.b1` | pfSense CE 2.7.2, FreeBSD 14 `amd64`, WireGuard 0.2.1 | Legacy experimental preview |

pfSense CE 2.8.1 has not been validated with either release. Do not install a
preview package on a different pfSense/FreeBSD/WireGuard combination merely
because installation succeeds.
...
Release `v0.1.0-beta.2` has been tested on pfSense CE 2.9.0, FreeBSD 16
`amd64`, with the official `pfSense-pkg-WireGuard` 0.2.13_4 package. Install
and configure that WireGuard package before installing Client Export. The
GitHub tag follows Semantic Versioning (`v0.1.0-beta.2`), while the equivalent
FreeBSD package version is `0.1.0.b2`.
...
```sh
cd /tmp
PACKAGE=pfSense-pkg-WireGuard-ClientExport-0.1.0.b2.pkg
RELEASE_URL=https://github.com/sirius0/pfsense-pkg-wireguard-client-export/releases/download/v0.1.0-beta.2
...
For this experimental preview, rollback means removing only the Client Export
companion package:
