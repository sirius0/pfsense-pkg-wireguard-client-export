...
{
    return $GLOBALS['wgce_wireguard_state']['configured_names'];
}

function wg_tunnel_get_config_by_name(string $tunnel): mixed
{
    return $GLOBALS['wgce_wireguard_state']['tunnels'][$tunnel] ?? false;
}
...
wgce_test('adapter enumerates tunnels through the official API without legacy globals', static function (): void {
    wgce_reset_wireguard_state();
    $GLOBALS['wgce_wireguard_state']['configured_names'] = ['tun_wg0'];
    $GLOBALS['wgce_wireguard_state']['tunnels']['tun_wg0'] = [3, [
        'name' => 'tun_wg0',
        'descr' => 'Remote access',
        'enabled' => 'yes',
        'publickey' => wgce_synthetic_key('S'),
        'listenport' => '443',
        'addresses' => ['row' => [['address' => '10.66.66.1', 'mask' => '24']]],
    ], false];

    $readiness = wgclientexport_wireguard_load(false);
    $tunnels = wgclientexport_list_tunnels();

    wgce_assert_true($readiness['ready']);
    wgce_assert_same('wg-0.2-compatible', $readiness['contract']);
    wgce_assert_same(1, count($tunnels));
    wgce_assert_same('tun_wg0', $tunnels[0]['name']);
    wgce_assert_same('443', (string) $tunnels[0]['listen_port']);
    wgce_assert_same(['row' => [['address' => '10.66.66.1', 'mask' => '24']]],
        $tunnels[0]['addresses']);
});

wgce_test('adapter rejects a configured tunnel name without a valid API row', static function (): void {
    wgce_reset_wireguard_state();
    $GLOBALS['wgce_wireguard_state']['configured_names'] = ['tun_wg0'];
